...
class TrainingStabilityTests(unittest.TestCase):
    ...
    def test_sqdf_clip_reward_moves_model_and_buffers_to_device(self):
        fake_open_clip = SimpleNamespace(
            create_model_and_transforms=lambda *args, **kwargs: (
                torch.nn.Linear(2, 2), None, None
            ),
            get_tokenizer=lambda *args, **kwargs: object(),
        )
        with patch.dict("sys.modules", {"open_clip": fake_open_clip}):
            reward = DifferentiableCLIPReward(torch.device("meta"))

        self.assertEqual(reward.mean.device.type, "meta")
        self.assertEqual(reward.std.device.type, "meta")
        self.assertTrue(
            all(parameter.device.type == "meta" for parameter in reward.parameters())
        )
