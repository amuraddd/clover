"""EMOV5: diffusion policy optimization with accumulated diversity replay.

Converted from ``clover/exp/ddpo.ipynb``. Run with
``python -m clover.baselines.emo_v5``.
"""
...
class EMOV2V2Config:
    ...
    # Per-latent-dimension cross entropy to a snapshot refreshed at each sac_update.
    cross_entropy_coefficient: float = 0.01
    ...
    def __post_init__(self) -> None:
        if type(self.buffer_reset) is not int or self.buffer_reset <= 0:
            raise ValueError("buffer_reset must be a positive integer")
        if not math.isfinite(self.cross_entropy_coefficient) or self.cross_entropy_coefficient < 0:
            raise ValueError("cross_entropy_coefficient must be finite and non-negative")
        if not math.isfinite(self.entropy_scale) or self.entropy_scale < 0:
...
def _emo_v5_step_with_log_prob(
    scheduler: Any, model_output: Tensor, timestep: int, sample: Tensor,
...
def collect_rollouts(
    ...
    for timestep_tensor in pipe.scheduler.timesteps:
        ...
        noise_pred = _predict_emo_v5_chunked(
            pipe,
            latents,
            timestep_tensor,
            prompt_embeds,
        ...
        next_latents, log_prob = _emo_v5_step_with_log_prob(
...
def _evaluate_emo_v5(
    ...
    images = _generate_emo_v5_eval_images(pipe, prompts, config, device, dtype)
...
def emo_v5_combined_rollouts(
    ...
    Args:
        ...
        trajectories: Optional mapping from integer-like rollout numbers to
            ...
            from ``trajectory_path`` or the default EMOV5 trajectory file.
        ...
        buffer_reset: Clear replay after this many completed epochs (default 5).
            Epochs 6, 11, ... start fresh with the default interval.
        epoch: Current one-based epoch. Defaults to required_trajectory_epoch + 1
            when supplied, otherwise 1. Reset occurs before loading replay,
            including when resuming at a reset boundary. An explicitly supplied
            in-memory buffer is also cleared; only the active replay file is deleted.
    ...
    """
    if type(buffer_reset) is not int or buffer_reset <= 0:
        raise ValueError("buffer_reset must be a positive integer")
    if epoch is None:
        epoch = int(required_trajectory_epoch) + 1 if required_trajectory_epoch is not None else 1
    if type(epoch) is not int or epoch < 1:
        raise ValueError("epoch must be a positive integer")
    if epoch > 1 and (epoch - 1) % buffer_reset == 0:
        ...
        if trajectory_path is not None or trajectories is None:
            Path(trajectory_path or "clover/data/emo_v5/trajectories.pt").unlink(missing_ok=True)
    ...
    if trajectories is None:
        try:
            trajectory_path = trajectory_path or "clover/data/emo_v5/trajectories.pt"
...
def _diagonal_gaussian_cross_entropy(
    mean: Tensor, variance: Tensor, reference_mean: Tensor, reference_variance: Tensor,
) -> Tensor:
    """H(current, reference) = -E_current[log reference], per latent dimension.

    The reference is detached; current mean and variance receive gradients.
    """
    ...
    if any(not torch.isfinite(t).all() for t in (mean, variance, reference_mean, reference_variance)):
        raise FloatingPointError("non-finite Gaussian cross entropy inputs")
    if (variance <= 0).any() or (reference_variance <= 0).any():
        raise FloatingPointError("Gaussian cross entropy requires positive variances")
    cross_entropy = 0.5 * (
        math.log(2.0 * math.pi) + reference_variance.log()
        + (variance + (mean - reference_mean).square()) / reference_variance
    )
    result = cross_entropy.flatten(1).mean(1)
    if not torch.isfinite(result).all():
        raise FloatingPointError("non-finite Gaussian cross entropy")
    return result
...
def sac_update(
    ...
    if not math.isfinite(config.cross_entropy_coefficient) or config.cross_entropy_coefficient < 0:
        raise ValueError("cross_entropy_coefficient must be finite and non-negative")
    if config.cross_entropy_coefficient > 0 and (not math.isfinite(config.eta) or config.eta <= 0):
        raise ValueError("Cross-entropy regularization requires finite positive eta")
    if not math.isfinite(config.entropy_scale) or config.entropy_scale < 0:
        raise ValueError("entropy_scale must be finite and non-negative")
    ...
    if batch_size == 0:
        return {
            "loss": float("nan"),
            "policy_objective": float("nan"),
            "entropy_objective": float("nan"),
            "entropy_bonus": float("nan"),
            "entropy_scale": float(config.entropy_scale),
            "cross_entropy_objective": float("nan"),
            "cross_entropy_penalty": float("nan"),
    ...
    reference_pipe = None
    if config.cross_entropy_coefficient > 0:
        ...
        for start in range(
            ...
            try:
                for step_idx, timestep in enumerate(timesteps):
                    ...
                    # -----------------------------------------------------
                    _, log_prob = _emo_v5_step_with_log_prob(
                        pipe.scheduler,
                        noise_pred,
                        timestep,
                        state,
                    ...
                    cross_entropy_to_reference = torch.zeros_like(log_prob)
                    if reference_pipe is not None:
                        if timestep <= 0:
                            raise ValueError("Cross entropy requires stochastic DDPM transitions")
                        with torch.no_grad():
                            reference_output = _predict_emo_v5_chunked(
                        ...
                        cross_entropy_to_reference = _diagonal_gaussian_cross_entropy(
                            current_mean, current_variance, reference_mean, reference_variance,
                        )

                    cross_entropy_objective = cross_entropy_to_reference.mean()
                    ...
                    step_loss.backward()
                    cross_entropy_values.append(float(cross_entropy_to_reference.detach().mean().cpu()))
    ...
    return {
        ...
        "policy_objective": metric_mean(policy_objectives),
        "entropy_objective": metric_mean(entropies),
        "entropy_bonus": metric_mean(entropy_bonuses),
        "cross_entropy_objective": metric_mean(cross_entropy_values),
        "cross_entropy_penalty": config.cross_entropy_coefficient * metric_mean(cross_entropy_values),
        "cross_entropy_coefficient": float(config.cross_entropy_coefficient),
        "entropy_scale": float(config.entropy_scale),
...
def train(
    ...
    if (
        ...
        raise RuntimeError("Failed to configure EMO-v5 learned_range scheduler")
    ...
    for epoch in trange(last_epoch + 1, config.train_epochs + 1):
        ...
        if last_epoch > 0 and epoch == last_epoch + 1 and not replay_path.exists():
            ...
            combined_rollout = emo_v5_combined_rollouts(
                ...
                buffer_reset=config.buffer_reset,
                epoch=epoch,
            )
        ...
        metrics.update(
            ...
            replay_source_epoch=epoch - 1 if replay_rewards.numel() else None,
            replay_buffer_reset=epoch % config.buffer_reset == 0,
            buffer_reset=config.buffer_reset,
        )
        metrics["epoch"] = epoch
        ...
        # Delete immediately after each completed block (5, 10, 15, ...).
        # The combiner also handles an interrupted/resumed boundary defensively.
        if epoch % config.buffer_reset == 0:
        ...
        if config.evaluate_every > 0 and epoch % config.evaluate_every == 0:
            _evaluate_emo_v5(pipe, config, device, dtype, epoch=epoch)
...
def main() -> None:
    """Parse command-line configuration and run EMOV5 training.

    Returns:
        None.
    """
    train(parse_config(EMOV2V2Config, __doc__ or "Train EMOV5"))
